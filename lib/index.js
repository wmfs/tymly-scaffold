...
module.exports = class Scaffold {
  ...
  _templateFile (filename) {
    return path.join(Scaffold._templateDir(), filename)
  }
  ...
  makeStateMachine (options) {
    ...
    const templateFile = `state-machines/${machineName}.json.ejs`

    const basename = options.filename || this._safeFilename(`${machineName}-${model}`)
    const filename = `state-machines/${basename}.json`
    ...
    this._templateCopy(templateFile, filename, ctx)
  }

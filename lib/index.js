...
module.exports = class Scaffold {
  ...
  addBlueprint (options) {
    ...
    this._templateCopy('test/test.ejs', 'test/test.js', {})
  ...
  addSeedData (options) {
    ...
    for (const [key, property] of Object.entries(modelSchema.properties)) {
      if (property.type !== 'array' && property.type !== 'object') {
        ...
        let suggestion = null
        if (Object.prototype.hasOwnProperty.call(property, 'examples')) {
          suggestion = property.examples[0]
        } else if (Object.prototype.hasOwnProperty.call(property, 'default')) {
          suggestion = property.default
        } else if (Object.prototype.hasOwnProperty.call(property, 'minimum')) {
  ...
  _makeForm (options, purpose) {
    ...
    if (purpose === 'viewing') {
      for (const property of Object.values(modelSchema.properties)) {
  ...
  makeSearchDoc (options) {
    ...
    options.primaryKeyString = pks
      .map(k => `${k}::text`)
      .join(' || ')
    options.descriptionString = buildDescription(options.description)
...
function buildDescription (description) {
  ...
  const allFields = snaked
    .join(', \' \', ')

  return `concat(${allFields})`
} // buildDescription

...
module.exports = class Scaffold {
  static ModelTypes () {
    return jsonSchemaMaker.TYPES.getDataTypeNames()
  }

  static StateMachines () {
    return fs.readdirSync(path.join(Scaffold._templateDir(), 'state-machines'))
  ...
  _templateFile (filename, componentDirName) {
    return componentDirName
      ? path.join(Scaffold._templateDir(), componentDirName, filename)
      : path.join(Scaffold._templateDir(), filename)
  }
  ...
  addModel (options) {
    const model = jsonSchemaMaker.dslToJsonSchema(options)
    this._extendFile(
  ...
  makeViewable (options) {
    this._makeForm(options, 'view')
  }

  _makeForm (options, fileNameSuggestion) {
    ...
    const filename = options.filename ||
      `${this._safeFilename(options.modelName)}-${fileNameSuggestion}-form.json`
